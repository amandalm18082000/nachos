...
int main()
{
	...
	// Input file name 1
	PrintString("Enter Source file's 1 name: ");
	...
	{
		// Input file name 2
		PrintString("Enter Source file's 2 name:");
		...
		{
			fileSize = Seek(-1, openIDFile_2); // Get file size
			// PrintNum(fileSize);
			setZero = Seek(0, openIDFile_2); // Seek the pointer to file head to read
											 // Read each character in file and print
			tmp = Seek(-1, openIDFile_1);
			for (i = 0; i < fileSize; i++)
			{
				read = Read(dataFileCopy, 1, openIDFile_2);
				// PrintStr(character);
				Write(dataFileCopy, 1, openIDFile_1);
			}
		}
	}
	tmp = CloseFile(openIDFile_1);
	tmp = CloseFile(openIDFile_2);
	Halt();
